...
| Arquivos ZIP/PK3/PYZ | completa no candidato local da issue #49; PR 3 ainda não publicada | regressão local em Python 3.14 e 3.10: 665 testes de manutenção e cinco do site aprovados; 11 skips explícitos (dez Windows e um smoke de rede); casos nativos Windows e matriz da PR 3 pendentes no CI; smokes nativos dos runtimes separados |
...
1. Concluir a matriz da PR 3, inclusive os casos nativos Windows, antes de
   classificar a validação de SEC-01 como multiplataforma completa; a regressão
   local já passou em Python 3.14 e 3.10. Preservar a `0.7.1` enquanto o
   candidato não for publicado.
