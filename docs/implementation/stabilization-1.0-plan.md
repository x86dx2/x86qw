...
`f4da1c4a4547617e1217e35148ce55b2960b5557` da branch
`agent/rc-1.0-completion`. O candidato local `1.0.0-rc.3` desse commit passou
...
e a evidência durável. O candidato local `1.0.0-rc.3` passou os 25 casos no M3;
...
possuem placeholders deliberados e o harness M3 local passou `25/25` no
  candidato `1.0.0-rc.3`. O RC público já foi promovido, mas a implementação
...
5. revisar o commit local `f4da1c4` contra `origin/main`, mantendo separado o
