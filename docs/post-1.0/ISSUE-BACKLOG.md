...
| FUNC-001 | DONE/P1 | Implementar `x86qw doctor` read-only | S0-003 | 1.1 / owner-only (PR #191) |
| FUNC-002 | DONE/P1 | Gerar bundle de diagnóstico sanitizado e revisável | FUNC-001 | 1.1 / owner-only (PR #191) |
| FUNC-003 | DONE/P1 | Separar perfis, defaults, cache e dados pessoais | S0-003 | 1.1 / owner-only (PR #193) |
| FUNC-004 | DONE/P1 | Favoritos e recentes locais com origem e freshness | FUNC-003 | 1.1 / owner-only (PR #194) |
| FUNC-005 | DONE/P1 | Descoberta e entrada em partidas com fallback local | FUNC-004 | 1.1 / owner-only (PR #194) |
| FUNC-006 | DONE/P1 | Presets declarativos de hospedagem local | S0-003 | 1.2 / owner-only (PR #195) |
| FUNC-007 | DONE/P1 | Readiness, logs, stop e crash recovery de servidor | FUNC-006 | 1.2 / owner-only (`status` / `--stop` / journal) |
...
2. F1–F3 e FUNC-008 já estão na main como **1.1 não publicado** (sem zipapp).
3. O delta in-tree de `host`/launcher/não-KTX é classe **1.0.1**; não constrói
   nem promove instalador.
