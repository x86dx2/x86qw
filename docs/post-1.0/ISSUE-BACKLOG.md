...
| S0-002 | DONE/P1 | Verificar convergência da projeção site, bootstraps, product e release-truth | S0-001 | documental/deploy protegido / owner-only (receipt 33136179763) |
...
2. F1–F3, FUNC-008 e o TUI polido entram na fonte **1.0.4** (sem linha 1.1 nesta fase).
3. A `1.0.0` publicada e as versões anteriores deixam de ser current; permanecem históricas no catálogo.
...
- `#146`: issue encerrada para o gate registrado; migração continua condicional a external-public.
- `#148`: issue encerrada após o drill técnico; custódia, backup, RTO e sucessão continuam requisitos external-public.
- `#150`: issue encerrada após preservar refs de evidência; não apagar novas refs sem revalidar ancestralidade.
- `#151`: issue encerrada como avaliação futura de imutabilidade host-level; não bloqueia funcionalidades M3.
