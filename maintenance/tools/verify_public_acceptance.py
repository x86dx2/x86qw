...
EXTERNAL_USER_OPERATIONS = ("migrate_dry_run", *BASE_OPERATIONS)
ACCEPTANCE_SCOPES = frozenset({"single-user", "external-users"})
...
def verify_record(
    ...
    *,
    ...
    acceptance_scope = record.get("acceptance_scope", "external-users")
    if acceptance_scope not in ACCEPTANCE_SCOPES:
        raise PublicAcceptanceError("recibo público possui acceptance_scope inválido")
    if expected_scope is not None:
        if expected_scope not in ACCEPTANCE_SCOPES:
            raise PublicAcceptanceError("escopo esperado do recibo é inválido")
        if acceptance_scope != expected_scope:
            raise PublicAcceptanceError("recibo público possui escopo diferente do gate")
    ...
    if record_format == 2 and acceptance_scope == "external-users":
    ...
    return {
        ...
        "migration_source_version": (
            "0.7.13" if record_format == 2 and acceptance_scope == "external-users" else None
        ),
    }
...
def main(arguments: list[str] | None = None) -> int:
    ...
    parser.add_argument("--expected-scope", choices=sorted(ACCEPTANCE_SCOPES))
