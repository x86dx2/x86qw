...
class ReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_patch_mirror_publication_is_protected_and_fail_closed(self):
        ...
        self.assertIn("git diff --diff-filter=AM --name-only", source)
        self.assertIn("for delta_path in", source)
        self.assertIn("git remote add public-gitlab", source)
        self.assertIn('git lfs push --object-id public-gitlab "$delta_sha"', source)
        self.assertIn("urllib.request.urlopen", source)
        self.assertNotIn("x86qw-installer/1.0.7", source)
