...
from maintenance.tools.verify_tuf_snapshot_publication import (
    SnapshotPublicationVerificationError,
    verify_snapshot_publication_receipt,
)


class VerifyTufSnapshotPublicationTests(unittest.TestCase):
    def _fixture(self, workspace: Path) -> tuple[Path, dict[str, object], dict[str, Path]]:
        ...
        snapshot_key_id = "a" * 64
        timestamp_key_id = "e" * 64
        changed_files = ["metadata/2.snapshot.json", "metadata/timestamp.json"]
        renewal_report = {
            "format": 1,
            "project": "x86qw",
        ...
        receipt = {
            ...
            "renewal": {
                ...
                "run_id": "500",
                "artifact_id": "600",
                "artifact_name": f"tuf-snapshot-renewal-{candidate_commit}-500-1",
    ...
    def test_valid_receipt_binds_all_candidate_and_public_verification_bytes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual("verified-snapshot-publication", result["status"])
    ...
    def test_candidate_digest_drift_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(SnapshotPublicationVerificationError, "catálogo"):
                verify_snapshot_publication_receipt(
                    receipt=receipt,
                    candidate=candidate,
                    renewal_report=files["renewal"],
    ...
    def test_timestamp_only_changed_files_are_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            candidate, receipt, files = self._fixture(Path(temporary))
            receipt["changed_files"] = ["metadata/timestamp.json"]
            with self.assertRaisesRegex(SnapshotPublicationVerificationError, "snapshot"):
                verify_snapshot_publication_receipt(
                    receipt=receipt,
                    candidate=candidate,
                    renewal_report=files["renewal"],
