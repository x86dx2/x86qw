...
class Release077Tests(unittest.TestCase):
    def test_release_identity_root_and_previous_bundle_are_consistent(self) -> None:
        ...
        self.assertEqual("0.7.7", (ROOT / "dist/installer/VERSION").read_text().strip())
        self.assertEqual("0.7.7", (ROOT / "dist/installer/packages/latest").resolve().name)
        self.assertEqual(["0.7.7"], [package["version"] for package in current])
        self.assertEqual("0.7.7", product["version"])
        ...
        bundle = ROOT / "dist/installer/packages/0.7.7/x86qw-installer-0.7.7.zip"
        ...
        with zipfile.ZipFile(bundle) as outer:
            application = outer.read("x86qw-installer-0.7.7/x86qw.pyz")
        ...
        self.assertIn('INSTALLER_VERSION="0.7.7"', (ROOT / "site/public/install.sh").read_text())
        self.assertIn('$InstallerVersion = "0.7.7"', (ROOT / "site/public/install.ps1").read_text())
    ...
    def test_public_text_identifies_the_large_package_hotfix(self) -> None:
        notes = (ROOT / "docs/releases/0.7.7.md").read_text(encoding="utf-8")
        self.assertIn("# x86QW 0.7.7", notes)
        self.assertIn("QRP", notes)
        self.assertIn("512 MiB", notes)
        index = (ROOT / "site/public/index.html").read_text(encoding="utf-8")
        self.assertIn("Distribuição 0.7.7 pública e verificável", index)
        self.assertIn("versão 0.7.7, 67 pacotes", index)
